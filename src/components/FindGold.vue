<template>
  <div class="card-panel green lighten-4">
    ...
    <p class="teal-text text-darken-3">(genera{{ oro }} <img src="coin.png">)</p>
...
export default {
  ...
  methods: {
    generarOro() {
      ...
      Store.state.actividades = [...Store.state.actividades, `${((newScore < 0) ? 'Perdiste' : 'Ganaste')} <span>${newScore}</span> <img src="coin.png"> ${((this.titulo == 'Casino') ? 'en el' : 'en la')} <b>${this.titulo}</b>${((newScore < 0) ? ' :(' : '!')} (${fechaActual})`];
    }
  }
};
</script>
